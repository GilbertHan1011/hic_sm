...
echo -e -n "valid_interaction\t" > ${prefix}.allValidPairs.mergestat
cat ${vpairs} | wc -l >> ${prefix}.allValidPairs.mergestat
echo -e -n "valid_interaction_rmdup\t" >> ${prefix}.allValidPairs.mergestat
cat ${prefix}.allValidPairs | wc -l >> ${prefix}.allValidPairs.mergestat

## Count short range (<20000) vs long range contacts
awk 'BEGIN{cis=0;trans=0;sr=0;lr=0} $2 == $5{cis=cis+1; d=$6>$3?$6-$3:$3-$6; if (d<=20000){sr=sr+1}else{lr=lr+1}} $2!=$5{trans=trans+1}END{print "trans_interaction\t"trans"\ncis_interaction\t"cis"\ncis_shortRange\t"sr"\ncis_longRange\t"lr}' ${prefix}.allValidPairs >> ${prefix}.allValidPairs.mergestat
